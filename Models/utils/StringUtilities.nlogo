...
households-own[
  ...
  occuped-flat      ;pointer to the occuped flat
]
...
to draw-gis-layers
  ...
  foreach gis:feature-list-of building-layer-data [if gis:property-value ? "ID" = target-id [gis:set-drawing-color red] gis:fill ? 1 gis:set-drawing-color black]
...
to set-flats-and-buildings
  ...
  create-flats 1 [setxy 0 0 set shape "house" set size 0.5 set color yellow]
  ...
  while [not filled?][
     ...
     let fixed? false
     ask buildings [ask current-flat [ifelse not fixed? [
           if gis:contains? [gis-shape] of myself self [hatch-flats 1 [set previous-flat current-flat set current-flat self] set fixed? true]] 
         [ if gis:contains? [gis-shape] of myself self [ask previous-flat [die]]  ;;two and only two max containing shape?   
           ] ]]
